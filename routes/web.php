...
Route::prefix('bendahara')->middleware(['auth', BendaharaMiddleware::class])->group(function() {
    ...
    Route::controller(ManageEmployeeController::class)->group(function() {
        ...
        Route::post('/simpan/karyawan', 'store')->name('manage.employee.store'); // Menyimpan Data
        Route::get('/edit/karyawan{id}', 'edit')->name('manage.employee.edit'); // Menampilkan Form Edit
        Route::put('/update/karyawan{id}', 'update')->name('manage.employee.update'); // Menampilkan Form Update
